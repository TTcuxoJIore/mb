...
try {
    ...
    musicbot.playFunction = (msg, suffix, args, ignore) => {
      //console.log(msg.channel.guild.voiceStates.cache.get(msg.author.id))
      let channel = msg.channel.guild.voiceStates.cache.get(msg.author.id)
      if (channel == undefined) return msg.channel.send(musicbot.note('fail', `You're not in a voice channel.`));
      if (!suffix) return msg.channel.send(musicbot.note('fail', 'No video specified!'));
      let q = musicbot.getQueue(msg.guild.id);

      console.log(channel.channelID)
      let vc = client.channels.cache.get(channel.channelID);
      //let vc = client.voice.connections.find(val => val.channel.guild.id == msg.member.guild.id)
      //if (vc && vc.channel.id != msg.channel.guild.voiceStates.cache.get(msg.author.id).channelID) return msg.channel.send(musicbot.note('fail', `You must be in the same voice channel as me.`));
      if (!vc) return msg.channel.send(musicbot.note('fail', `I can\`t invite this voice channel.`));
          ...
          if (!ignore) {
            if (msg.channel.permissionsFor(msg.guild.me).has('EMBED_LINKS')) {
              ...
              try {
                ...
                console.log(client.users.cache.get(res.requester))
    ...
    musicbot.skipFunction = (msg, suffix, args) => {
      if (!msg.channel.guild.voiceStates.cache.get(msg.author.id)) return msg.channel.send(musicbot.note('fail', `You're not in a voice channel.`));
      const voiceConnection = client.voice.connections.find(val => val.channel.guild.id == msg.guild.id);
      if (voiceConnection == undefined) return msg.channel.send(musicbot.note('fail', 'No music being played.'));
      if (voiceConnection && voiceConnection.channel.id != msg.channel.guild.voiceStates.cache.get(msg.author.id).channelID) return msg.channel.send(musicbot.note('fail', `You must be in the same voice channel as me.`));
    ...
    musicbot.pauseFunction = (msg, suffix, args) => {
      ...
      const dispatcher = voiceConnection.player.dispatcher;
      if (!dispatcher) return msg.channel.send(musicbot.note(`fail`, `Music already ended!`))
    ...
    musicbot.leaveFunction = (msg, suffix) => {
      if (musicbot.isAdmin(msg.member) || musicbot.anyoneCanLeave === true) {
        if (!msg.channel.guild.voiceStates.cache.get(msg.author.id)) return msg.channel.send(musicbot.note('fail', `You're not in a voice channel.`));
        const voiceConnection = client.voice.connections.find(val => val.channel.guild.id == msg.guild.id);
        if (voiceConnection == undefined) return msg.channel.send(musicbot.note('fail', 'I\'m not in a voice channel.'));
        if (voiceConnection && voiceConnection.channel.id != msg.channel.guild.voiceStates.cache.get(msg.author.id).channelID) return msg.channel.send(musicbot.note('fail', `You must be in the same voice channel as me.`));
    ...
    musicbot.npFunction = (msg, suffix, args) => {
      ...
      if (msg.channel.permissionsFor(msg.guild.me)
        ...
        try {
          ...
          console.log(client.users.cache.get(queue.last.requester))
          ...
          msg.channel.send({
            ...
            m.react('⏸').then(() => {
              m.react('⏩')
              let pauseFilter = m.createReactionCollector((reaction, user) => reaction.emoji.name === '⏸' && user.id === msg.author.id, { time: 120000 });
              let skipFilter = m.createReactionCollector((reaction, user) => reaction.emoji.name === '⏩' && user.id === msg.author.id, { time: 120000 });

              skipFilter.on('collect', (e, u) => {
                if (!msg.channel.guild.voiceStates.cache.get(msg.author.id)) return
                const voiceConnection = client.voice.connections.find(val => val.channel.guild.id == msg.guild.id);
                if (voiceConnection == undefined) return
                if (voiceConnection && voiceConnection.channel.id != msg.channel.guild.voiceStates.cache.get(msg.author.id).channelID) return

                const queue = musicbot.getQueue(msg.guild.id);
                if (!musicbot.canSkip(msg.member, queue)) return

                if (musicbot.queues.get(msg.guild.id).loop == "song") return
              ...
              console.log(Discord)
              pauseFilter.on('remove', (e, u) => {
                console.log("I`m alive too")
                if (!msg.channel.guild.voiceStates.cache.get(msg.author.id)) return
                const voiceConnection = client.voice.connections.find(val => val.channel.guild.id == msg.guild.id);
                if (voiceConnection == undefined) return
                if (voiceConnection && voiceConnection.channel.id != msg.channel.guild.voiceStates.cache.get(msg.author.id).channelID) return
                if (!musicbot.isAdmin(msg.member) && !musicbot.anyoneCanPause) return
    ...
    musicbot.searchFunction = (msg, suffix, args) => {
      if (msg.channel.guild.voiceStates.cache.get(msg.author.id) === undefined) return msg.channel.send(musicbot.note('fail', `You're not in a voice channel.`));
      let vc = client.voice.connections.find(val => val.channel.guild.id == msg.member.guild.id)
      if (vc && vc.channel.id != msg.channel.guild.voiceStates.cache.get(msg.author.id).channelID) return msg.channel.send(musicbot.note('fail', `You must be in the same voice channel as me.`));
    ...
    musicbot.executeQueue = (msg, queue) => {
      ...
      new Promise((resolve, reject) => {
          const voiceConnection = client.voice.connections.find(val => val.channel.guild.id == msg.guild.id);
          if (voiceConnection == null) {
            if (msg.channel.guild.voiceStates.cache.get(msg.author.id) && msg.channel.guild.voiceStates.cache.get(msg.author.id).joinable) {
              msg.channel.guild.voiceStates.cache.get(msg.author.id).join()
                .then(connection => {
                  resolve(connection);
                })
                .catch((error) => {
            ...
            } else if (!msg.channel.guild.voiceStates.cache.get(msg.author.id).joinable || msg.channel.guild.voiceStates.cache.get(msg.author.id).full) {
          ...
          if (false) if (musicbot.messageNewSong == true && queue.last && queue.loop !== "song") {
            ...
            if (msg.channel.permissionsFor(msg.guild.me).has('EMBED_LINKS')) {
              ...
              msg.channel.send({embed}).catch((res) => {
                msg.channel.send(musicbot.note("fail", "Something went wrong next track!"))
    ...
    client.on('raw', packet => {
      ...
      if (channel.messages.has(packet.d.message_id)) return console.log('I haven`t');
      ...
      channel.fetchMessage(packet.d.message_id).then(message => {
          ...
          if (packet.t === 'MESSAGE_REACTION_REMOVE') {
              console.log('I emit remove')
              client.emit('remove', reaction, client.users.get(packet.d.user_id));
          }
      });
    });
